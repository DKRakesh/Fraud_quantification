...
dimension=MODEDat.dim;
HO=MODEDat.Ho;
label=MODEDat.label;
error_min=MODEDat.error_min;
feat_min=MODEDat.feat_min;
...
%}
Parent=zeros(Xpop,dimension);
Mutant=zeros(Xpop,dimension);
Child=zeros(Xpop,dimension);
...
%disp('JxParent');
%disp(JxParent);
[error_min,idx]=min(JxParent(:,(2)));
MODEDat.error_min=error_min;
%disp('minimum');
%disp(error_min);
feature_temp=JxParent(:,(1));
feat_min=feature_temp(idx);
MODEDat.feat_min=feat_min;
%disp(feat_min);
% my code
...
for n=1:Generaciones 
    ...
    for xpop=1:Xpop
        ...
        A1 = {P1,P2,P3};
        Pvert = cat(1,A1{:});
        Jbest=mop(Pvert,MODEDat);
        %disp('Jbest');
        %disp(Jbest);
        [Best,idx]=sortrows(Jbest);
        %disp('Best');
        %disp(Best);
        %disp(idx(1));
        %disp(A1(idx(1)));
        %disp(Parent(rev(1,idx(1)),:));
        %% Mutant vector calculation
        Mutant(xpop,:)= Parent(rev(1,idx(1)),:)+ScalingFactor*(Parent(rev(1,idx(2)),:)-Parent(rev(1,idx(3)),:));
        disp(Parent(rev(1,1),:));
        %{
        disp('mutant');
        disp(Parent(rev(1,1),:));
